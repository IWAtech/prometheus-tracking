...
export class Tracker {
  ...
  public static getInstance(name: string, port: number = 9090, collectDefaultMetrics: boolean = true) {
    return new Tracker(name, port);
  }

  private constructor(private name: string, port: number, collectDefaultMetrics: boolean = true) {
  ...
  public trackResponseTime(uri: string, status: number, seconds: number) {
    this.responseTime.labels(this.name, uri, status.toString()).observe(seconds);
    this.requestCounter.labels(this.name, uri, status.toString()).inc();
  }
